...
describe("Check multipliers", function () {
    ...
    await this.MasterChefV2.add(0, this.DUMMY.address, "0x0000000000000000000000000000000000000000")
  ...
  it("Alice pending for SLP pool should be not 0", async function (){
    ...
    const timestamp0 = (await this.signers[0].provider.getBlock(blockNumber)).timestamp;
    	    console.log(await this.OpenX.balanceOf(this.alice.address))

    await this.MasterChefV2.harvest(1, this.alice.address)
   	console.log(await this.OpenX.balanceOf(this.alice.address))

    let BobPending = await this.MasterChefV2.pendingReward(1, this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    ...
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
    await this.MasterChefV2.deposit(1, ethers.utils.parseUnits('1', 18), this.alice.address)
	BobPending = await this.MasterChefV2.pendingReward(1, this.alice.address)
	    await this.MasterChefV2.harvest(1, this.alice.address)

	    console.log(await this.OpenX.balanceOf(this.alice.address))


    const blockNumber1 = (await this.signers[0].provider.getBlockNumber());
    const timestamp01 = (await this.signers[0].provider.getBlock(blockNumber1)).timestamp;
    
    //let BobPending = await this.MasterChefV2.pendingReward(1, this.alice.address)

	console.log("startTime : " + startTime)
	console.log("current time at 0: " + timestamp0 )
	console.log("current time at 1: " + timestamp01 )
	console.log("cbob: " + BobPending )
